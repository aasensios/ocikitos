...
export class IncidentsComponent implements OnInit {
  ...
  filter(): void {
    ...
    // as a parameter
    this.incidentsFiltered = this.incidents.filter(incident => {
      ...
      if (this.locationFilter && this.locationFilter !== '') {
        // filter by chip
        locationValid = incident.location.toLowerCase().indexOf(this.locationFilter.toLowerCase()) !== -1
      } else {
        locationValid = true
      }
      ...
      if (this.barcodeFilter && this.barcodeFilter !== '') {
        // filter by name
        barcodeValid = incident.sample_barcode.toLowerCase().indexOf(this.barcodeFilter.toLowerCase()) !== -1
      } else {
        barcodeValid = true
      }
